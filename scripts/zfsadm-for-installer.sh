...
SPL_CONFIGURE_ARRAY+=(--enable-boot)
...
ZFS_CONFIGURE_ARRAY+=(--enable-boot)
...
then
	...
	ZFS_CONFIGURE_ARRAY+=(--sbindir=/usr/local/bin)
	SPL_CONFIGURE_ARRAY+=\
(${HIGHSIERRA_HEADERS:+--with-kernel-headers="$HIGHSIERRA_HEADERS"})
	ZFS_CONFIGURE_ARRAY+=\
(${HIGHSIERRA_HEADERS:+--with-kernelsrc="$HIGHSIERRA_HEADERS"})
	ZFS_CONFIGURE_ARRAY+=\
(--with-filesystems-prefix=/Library/Filesystems)
elif [ x"$TARGET_OS_X_VERSION" != x"native" ]
then
	$ECHO "target should be '10.8', '10.9', '10.10', '10.11', '10.12', '10.13', or 'native'"
	exit 22
fi
